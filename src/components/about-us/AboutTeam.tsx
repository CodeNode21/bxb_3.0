import Link from "next/link";



export default function AboutTeam() {
  return (
    <section className="bxb-section-padding2">
      <div className="container">
        <div className="bxb-section-title two-column" data-aos="fade-right" data-aos-delay="500">
          <div className="row">
            <div className="col-lg-7">
              <h2>Our team consists of a group of talents</h2>
            </div>
            <div className="col-lg-5 d-flex align-items-center">
              <div className="section-title-button">
                <Link className="bxb-default-btn" data-aos="fade-right" data-aos-delay="900" href="/team" data-text="Join our team">
                  <span className="button-wraper">Join our team</span>
                </Link>
              </div>
            </div>
          </div>
        </div>
        <div className="row">
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="bxb-team-wrap wrap2" data-aos="fade-up" data-aos-delay="500">
              <Link href="/single-team">
                <div className="bxb-team-thumb">
                  <img src="/assets/images/team/team1.png" alt="Thumb" />
                </div>
                <div className="bxb-team-content">
                  <h3>Mr. Abraham Maslo</h3>
                </div>
              </Link>

              <div className="bxb-team-author-wrap">
                <div className="bxb-team-author-data">
                  <p>Chief AI Officer</p>
                </div>
                <div className="bxb-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="/assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="/assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="/assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="/assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="bxb-team-wrap wrap2" data-aos="fade-up" data-aos-delay="600">
              <Link href="/single-team">
                <div className="bxb-team-thumb">
                  <img src="/assets/images/team/team2.png" alt="Thumb" />
                </div>
                <div className="bxb-team-content">
                  <h3>Willium Robert</h3>
                </div>
              </Link>

              <div className="bxb-team-author-wrap">
                <div className="bxb-team-author-data">
                  <p>Data Engineer</p>
                </div>
                <div className="bxb-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="/assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="/assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="/assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="/assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="bxb-team-wrap wrap2" data-aos="fade-up" data-aos-delay="900">
              <Link href="/single-team">
                <div className="bxb-team-thumb">
                  <img src="/assets/images/team/team3.png" alt="Thumb" />
                </div>
                <div className="bxb-team-content">
                  <h3>Henry Fayol</h3>
                </div>
              </Link>
              <div className="bxb-team-author-wrap">
                <div className="bxb-team-author-data">
                  <p>Research Scientist</p>
                </div>
                <div className="bxb-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="/assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="/assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="/assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="/assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>

          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="bxb-team-wrap wrap2" data-aos="fade-up" data-aos-delay="500">
              <Link href="/single-team">
                <div className="bxb-team-thumb">
                  <img src="/assets/images/team/team4.png" alt="Thumb" />
                </div>
                <div className="bxb-team-content">
                  <h3>Henry Martine</h3>
                </div>
              </Link>
              <div className="bxb-team-author-wrap">
                <div className="bxb-team-author-data">
                  <p>AI Researchers</p>
                </div>
                <div className="bxb-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="/assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="/assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="/assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="/assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="bxb-team-wrap wrap2" data-aos="fade-up" data-aos-delay="600">
              <Link href="/single-team">
                <div className="bxb-team-thumb">
                  <img src="/assets/images/team/team5.png" alt="Thumb" />
                </div>
                <div className="bxb-team-content">
                  <h3>Jack Fox</h3>
                </div>
              </Link>
              <div className="bxb-team-author-wrap">
                <div className="bxb-team-author-data">
                  <p>NLP Expert</p>
                </div>
                <div className="bxb-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="/assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="/assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="/assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="/assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-4 col-lg-6 col-md-6">
            <div className="bxb-team-wrap wrap2" data-aos="fade-up" data-aos-delay="900">
              <Link href="/single-team">
                <div className="bxb-team-thumb">
                  <img src="/assets/images/team/team6.png" alt="Thumb" />
                </div>
                <div className="bxb-team-content">
                  <h3>Adam Smith</h3>
                </div>
              </Link>
              <div className="bxb-team-author-wrap">
                <div className="bxb-team-author-data">
                  <p>Project Manager</p>
                </div>
                <div className="bxb-social-wrap2 social-hover-orange">
                  <ul>
                    <li>
                      <a href="https://www.facebook.com">
                        <img src="/assets/images/home2/facebook.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.twitter.com">
                        <img src="/assets/images/home2/twitter.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.instagram.com">
                        <img src="/assets/images/home2/insta.svg" alt="Icon" />
                      </a>
                    </li>
                    <li>
                      <a href="https://www.linkedin.com">
                        <img src="/assets/images/home2/in.svg" alt="Icon" />
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
