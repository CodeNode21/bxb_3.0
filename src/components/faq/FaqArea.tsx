  ...
  return (
    <div className="bxb-section-padding">
      <div className="container">
        <div className="bxb-section-title center max-width-950" data-aos="fade-up" data-aos-delay="500">
          <h2>Our experts are able to answer all your questions</h2>
        </div>
        <div className="bxb-faq-wrap1 faq-page" data-aos="fade-up" data-aos-delay="600">
          {faq_data.map((item, i) => (
            <div key={i} className={`bxb-faq-item ${openIndex === i ? 'open' : ''}`}>
              <div className="bxb-faq-header" onClick={() => toggleAccordion(i)}>
                <h3>{item.question}</h3>
                <div className="bxb-active-icon">
                  <img src="/assets/images/home1/+.svg" alt="plas" />
                </div>
              </div>
              <div className="bxb-faq-body" style={{ display: openIndex === i ? 'block' : 'none' }}>
                <p>{item.answer}</p>
              </div>
            </div>
          ))}
